...
ANSWER 1: The code is establishing a giant set of random numbers. Then in the runif function
we are using numbers that are larger than 1. 
...
for(p in 1:3){
  ...
  time_each_took_to_run<-c(difference,time_each_took_to_run)

}
avg<-mean(time_each_took_to_run)
print(avg)
...
< i and J are telling us where it is in the dataset?>
...
time_each_took_to_run<-NULL
nrows<-1:nrow(sequencing_matrix)
ncols<-1:ncol(sequencing_matrix)
for(i in nrows){
   start_time<- Sys.time()
  for(j in ncols){
    if(sequencing_matrix[i,j]=="a"){sequencing_matrix[i,j]<-"A"}
...
for(i in 1:nrow(sequencing_matrix))
    for(j in 1:ncol(sequencing_matrix)){
      if(sequencing_matrix[i,j]=="t"){sequencing_matrix[i,j]<-"T"}
    }
